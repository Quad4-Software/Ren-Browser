...
async function main() {
    ...
    if (process.env.REN_BROWSER_OFFLINE_BUILD === "1") {
        if (!fs.existsSync(wasm) || !fs.existsSync(wasmExec)) {
            console.error(
                "fetch-micron-wasm: REN_BROWSER_OFFLINE_BUILD=1 but micron-parser-go.wasm or wasm_exec.js is missing."
            );
            process.exit(1);
        }
        console.log("fetch-micron-wasm: REN_BROWSER_OFFLINE_BUILD=1 and artifacts present, skipping fetch.");
        process.exit(0);
    }
