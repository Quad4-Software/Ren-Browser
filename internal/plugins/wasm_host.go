...
func (h *wasmHost) httpFetch(ctx context.Context, mod api.Module, reqPtr, reqLen, respPtr, respCap uint32) uint32 {
	if !h.networkFetch {
		return writeWasmHTTPError(mod, respPtr, respCap, "plugin lacks network.fetch permission")
	}
	reqBytes, ok := mod.Memory().Read(reqPtr, reqLen)
	if !ok {
		return writeWasmHTTPError(mod, respPtr, respCap, "invalid request memory")
	}
	var req WasmHTTPRequest
	if err := json.Unmarshal(reqBytes, &req); err != nil {
		return writeWasmHTTPError(mod, respPtr, respCap, "invalid request JSON")
	}
	resp, err := h.fetch(req)
	if err != nil {
		return writeWasmHTTPError(mod, respPtr, respCap, err.Error())
	}
	out, err := json.Marshal(resp)
	if err != nil {
		return writeWasmHTTPError(mod, respPtr, respCap, "marshal response failed")
	}
	if len(out) > int(respCap) {
		return writeWasmHTTPError(mod, respPtr, respCap, "response buffer too small")
	}
	if !mod.Memory().Write(respPtr, out) {
		return writeWasmHTTPError(mod, respPtr, respCap, "write response failed")
	}
	return uint32(len(out)) // #nosec G115 -- wasm ABI length
}

func writeWasmHTTPError(mod api.Module, respPtr, respCap uint32, message string) uint32 {
